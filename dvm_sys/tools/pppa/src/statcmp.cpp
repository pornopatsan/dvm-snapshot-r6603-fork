...
{
    //здесь будет вывод помощи для сравнении статистик
}


//stcmp <output_file> <stat1.gz+> ... <statN.gz+>
int main(int argv, char **argc)
{
    ...
    //new_handler set_new_handler(NULL);
		nparout = 1;// output file
		//nparin = 2;// statistics file


    // read time characteristics and syn times
    try {
				for (int nparin = 2; i < argv; ++i) {       //собираем вектор из CStatRead
					stats.push_back(CStatRead(argc[nparin])); // для каждого файла
					if (stat.Valid(&warn) != TRUE) {          // со статистикой
	            char t[80];
	            stat.TextErr(t);
	            printf("%s", t);
	            printHelp();
