...
DerivedAxisRule DvmPragmaHandler::parseDerivedAxisRule(Preprocessor &PP, Token &Tok) {
...
DistribRule DvmPragmaHandler::parseDistribRule(Preprocessor &PP, Token &Tok) {
    DistribRule res;
    std::string tokStr;
    while (Tok.is(tok::l_square)) {
        PP.LexNonComment(Tok);
...
AlignAxisRule DvmPragmaHandler::parseAlignAxisRule(Preprocessor &PP, Token &Tok, std::map<std::string, int> nameToAxis, bool parLoopFlag) {
    AlignAxisRule res;
...
AlignRule DvmPragmaHandler::parseAlignRule(Preprocessor &PP, Token &Tok, bool parLoopFlag) {
    AlignRule res;
    std::string tokStr;
    int rank = 0;
    while (Tok.is(tok::l_square)) {
...
std::vector<SlicedArray> DvmPragmaHandler::parseSubarrays(Preprocessor &PP, Token &Tok) {
...
MyExpr DvmPragmaHandler::readExpr(Preprocessor &PP, Token &Tok, std::string stopTokens) {
    MyExpr res;
    if (!stopTokens.empty()) {
        if (stopTokens[0] != ' ')
...
std::pair<MyExpr, MyExpr> DvmPragmaHandler::readRange(Preprocessor &PP, Token &Tok) {
    MyExpr lb, hb;
    lb = readExpr(PP, Tok, ":");
...
ClauseRemoteAccess DvmPragmaHandler::parseOneRma(Preprocessor &PP, Token &Tok, const std::map<std::string, int> &nameToAxis) {
